...
int main()
{
	const char *name = "OS_HW1_DanielCurtis"; // Name of shared memory block to be passed to shm_open
    ...
    int count = 0;            
    while(count < itemCnt) {              // loop through number of items
       
        in = GetIn();
        out = GetOut();
        while(in != out)  {                     // instead of do nothing, it does something   
           int  consVal = ReadAtBufIndex(out);
            printf("Consuming Item %d with value %d at Index %d \n", count, consVal, out);
            out = (out + 1) % bufSize;
            SetOut(out);
            count++;
