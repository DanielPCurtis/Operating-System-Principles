...
int main(int argc, char* argv[])
{
    ...
    if(bufSize > 1000) {
        printf("Buffer size is too large, please try again\n");
        exit(1);
    }
    if(bufSize < 2) {
        printf("Buffer size is too small, please try again\n");
        exit(1);
    }
...
void InitShm(int bufSize, int itemCnt)
{
    int in = 0;
    int out = 0;
	const char *name = "OS_HW1_DanielCurtis"; // Name of shared memory object to be passed to shm_open
    int shm_fd;
...
void Producer(int bufSize, int itemCnt, int randSeed)
{
	...
	// where i is the item number, val is the item value, in is its index in the bounded buffer
            ...
            myVal = GetRand(0,3000);
            WriteAtBufIndex(in, myVal);
            printf("Producing Item %d with value %d at Index %d \n", count, myVal, in);
            in = (in + 1) % bufSize;
            SetIn(in);
            count++;
